      ...
      if (e.code == 'invalid-email') {
        ...
        if (widget.basicSettings.agentRegistartionEnabled == true) {
          ...
          showERRORSheet(
            context,
            "124",
            message: getTranslatedForCurrentUser(context, 'xxemailnotfoundlxx')
                .replaceAll('(####)',
      ...
      if (e.toString().contains("no user") ||
        ...
        if (e.toString().contains("does not have a password")) {
          ...
          showERRORSheet(context, "EM_111",
              message:
                  "${getTranslatedForCurrentUser(context, 'xxfailedxx')} \n\n. ${getTranslatedForCurrentUser(context, 'xxcannotuseemailxx')}");
      ...
      await FirebaseFirestore.instance
        ...
        if (customers.docs.length > 0) {
          ...
          showERRORSheet(context, "EM_105",
              message:
                  "${getTranslatedForCurrentUser(context, 'xxfailedxx')}\n\n ${getTranslatedForCurrentUser(context, 'xxalreadyregistereddescemailxx').replaceAll('(###)', getTranslatedForCurrentUser(context, 'xxcustomerxx')).replaceAll('(##)', getTranslatedForCurrentUser(context, 'xxagentxx'))}");
        } else {
          if (agents.docs.length == 0) {
            if (widget.basicSettings.agentRegistartionEnabled == true) {
              ...
              showERRORSheet(context, "EM_108",
                  message:
                      "${getTranslatedForCurrentUser(context, 'xxfailedxx')}\n\n  ${getTranslatedForCurrentUser(context, 'xxregistrationdisabledxx')}");
            }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: MtCustomfontBoldSemi(
          text:
              "${getTranslatedForCurrentUser(context, 'xxagentxx')} ${getTranslatedForCurrentUser(context, 'xxaccountxx')}",
          fontsize: 17,
